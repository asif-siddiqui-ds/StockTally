  ...
  const panGesture = Gesture.Pan()
    ...
    .onEnd(() => {
      if (translateX.value < -100) {
        if (isConsumed) {
          runOnJS(onView)(record);
        } else {
          runOnJS(onDeleteSale)(record);
        }
      } else if (translateX.value > 100) {
        if (isConsumed) {
          runOnJS(onReverseConsumed)(record);
        } else {
          runOnJS(onTogglePaid)(record);
        }
      }
        ...
        >
          ...
          {isConsumed && typeof record.balanceAfter === "number" && (
            <Text style={styles.balanceText}>
              Balance after: {record.balanceAfter} {record.unit || "units"}
            </Text>
          )}
        </View>
      </Animated.View>
...
const StockOutHistoryScreen = () => {
    ...
    for (const sale of sales || []) {
      ...
      if (!groups[groupId]) {
        ...
        groups[groupId] = {
          ...
          paid: sale.paid ?? true,
          date: sale.date || sale.createdAt || sale.$createdAt || "",
          timestamp: parseTimestamp(
            sale.date || sale.createdAt || sale.$createdAt
          ),
          items: [],
        };
      }
  ...
  const sortRecords = useCallback(
    (records: HistoryRecord[], order: SortOrder) =>
      [...records].sort((a, b) =>
        order === "asc"
  ...
  const applyFilters = useCallback(
    (
      ...
      let filtered = base.filter((record) => {
      ...
      if (payment === "Paid") {
        filtered = filtered.filter(
          (record) =>
            record.recordType !== "consumed" && record.paid === true
        );
      } else if (payment === "Unpaid") {
        filtered = filtered.filter(
          (record) =>
            record.recordType !== "consumed" && record.paid === false
        );
      }

      setFilteredRecords(sortRecords(filtered, order));
    },
    [locale, sortRecords]
  );
  ...
  const loadRecords = useCallback(async () => {
    try {
      ...
      Alert.alert(
        "Error",
        "Could not load stock out history."
      );
    } finally {
      setRefreshing(false);
    }
  ...
  const summary = useMemo(() => {
    const saleCount = allRecords.filter(
      (record) => record.recordType !== "consumed"
    ).length;

    const consumedCount = allRecords.filter(
      (record) => record.recordType === "consumed"
    ).length;

    return { saleCount, consumedCount };
  }, [allRecords]);
  ...
  const handleMainFilter = (
    ...
    applyFilters(
      ...
      searchQuery,
      filter,
      filter === "Consumed" ? "All" : paymentFilter,
      sortOrder
    );
  ...
  const handleView = (record: HistoryRecord) => {
    if (record.recordType === "consumed") {
      router.push({
        pathname: "/screens/stock/viewStockOutScreen",
        params: {
          type: "consumed",
  ...
  const handleTogglePaid = async (record: HistoryRecord) => {
    ...
    try {
      ...
      const updated = sales.map((sale: any) => {
        const groupId =
          sale.batchId ||
          sale.salesId ||
          sale.saleId ||
        ...
        return String(groupId) === String(targetGroupId)
          ? { ...sale, paid: !record.paid }
          : sale;
      });
  ...
  const deleteSale = async (record: HistoryRecord) => {
    try {
      ...
      const remaining = sales.filter((sale: any) => {
        const groupId =
          sale.batchId ||
          sale.salesId ||
          sale.saleId ||
          ...
          onPress: async () => {
            try {
              ...
              await saveStockMovement({
                ...
                type: "IN",
                quantity: Number(record.totalItems || 0),
                source: "ADJUSTMENT",
                sourceLabel:
                  "Consumed stock reversed - stock restored",
                balanceAfter: restoredBalance,
                ...
                referenceType: "STOCK",
                note:
                  "Reversal of consumed in-house stock",
              });
        ...
        >
          <View style={styles.headerArea}>
            ...
            <Text style={styles.screenSubtitle}>
              Quick sales, bulk sales and stock consumed
              in-house.
            </Text>
          ...
          <View style={styles.filterArea}>
            ...
            <View style={styles.dropdownRow}>
              ...
              <Dropdown
                ...
                value={sortOrder}
                onChange={(item) =>
                  handleSortChange(item.value as SortOrder)
                }
              />
            </View>
          </View>
